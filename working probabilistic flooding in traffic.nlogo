globals [
  ...
  fromID ; origin point of the broadcast
  theoretical-max-nodes-reached ; max number of nodes the broadcast could reach (range * ttl)
  nodes-reached ; actual number of nodes reached
  perc-nodes-reached ; percentage of potential nodes reached
]
...
to broadcast
  ...
  ask selected-car [
    ...
    set theoretical-max-nodes-reached count turtles in-radius (comm-range * (ttl))
  ]
  loop [
    ask turtles with [color = red] [
      let senderID who
      let senderPacket packet
      if length packet <= ttl[
        ask turtles in-radius comm-range
        with [(color = white or color = black) and who != senderID and count my-links <= 1] [
          ifelse random 100 <= p [ print "decided to broadcast"
            create-link-from myself
            set nodes-reached nodes-reached + 1
            set packet lput who senderPacket
            if color = white [
              set color red
            ]
            if color = black [
          ...
          ][print "decided NOT to broadcast"]
        ]
      ]
      set color blue
    ]
    if not any? turtles with [color = red] [
      set perc-nodes-reached (nodes-reached / theoretical-max-nodes-reached) * 100
      stop
    ] ;added line
  ]
end
...
<experiments>
  <experiment name="experiment" repetitions="100" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>broadcast</go>
    <metric>perc-nodes-reached</metric>
    <metric>nodes-reached</metric>
    <steppedValueSet variable="number-of-cars" first="50" step="50" last="200"/>
    <steppedValueSet variable="p" first="0" step="5" last="100"/>
  </experiment>
</experiments>
@#$#@#$#@
@#$#@#$#@
