globals [
  ...
  num-v-reached ; number of vehicles reached by a packet
  packets-created ;number of packets created
...
cars-own [
  ...
  recieved-signal ; bool for if signal has already been recieved
  sent-signal ; bool for if signal has already been sent
]

breed [packets packet]

packets-own [
  signal-id ; unique identifier for this EWS
]
...
to go
  ...
  if (random 100 < emergency-chance) and (emergency-this-tick = 0) [ ; if this car has an emergency
    ask links [die]

    ask one-of cars [ ;pick a car

      ask cars [set color car-color]
      set emergency-this-tick 1
      set color orange

      set ttl start-ttl ; initialise time-to-live

      set packets-created packets-created + 1
      print "packet created"

      set recieved-signal 1 ; make it so the car starting the signal is marked as having recieved it
      broadcast
    ]
...
to broadcast
  ask cars in-radius comm-range with [recieved-signal = 0][ ;broadcast to all cars in range which haven't already recieved
    ...
    set color green ;visual indicator
    set recieved-signal 1 ; mark they have recieved it
  ]

  if ttl > 0 [
    set ttl ttl - 1
    print "ttl reduced by 1"
    print ttl

    ask cars with [recieved-signal = 1 and sent-signal = 0][
      set sent-signal 1
      broadcast
      print "re-broadcasting"
    ]
  ]
...
"" ""
PENS
"average" 1.0 0 -10899396 true "" "plot mean [ patience ] of cars"
"max" 1.0 0 -11221820 true "" "plot max [ patience ] of cars"
...
"" ""
PENS
"total packets created" 1.0 0 -16777216 true "" "plot packets-created"
"packets currently active" 1.0 0 -817084 true "" "plot count packets"
